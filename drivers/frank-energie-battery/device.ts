...
export = class SmartBatteryDevice extends FrankEnergieDeviceBase {
  ...
  async initializeDeviceSpecificClients(): Promise<void> {
    ...
    this.externalBatteryMetrics = new BatteryMetricsStore({
      device: this,
      logger: (msg, ...args) => this.log(msg, ...args),
      onBeforeAutomaticReset: (dailyCharged, dailyDischarged) => this.handleBeforeAutomaticBaselineReset(dailyCharged, dailyDischarged),
  ...
  /**
   * Handle BEFORE automatic baseline reset (called at 00:00 BEFORE reset)
   * Logs daily totals from the day that just ended
   */
  private async handleBeforeAutomaticBaselineReset(dailyChargedKwh: number, dailyDischargedKwh: number): Promise<void> {
    const dailyCharged = dailyChargedKwh.toFixed(2);
    const dailyDischarged = dailyDischargedKwh.toFixed(2);

    // Always log to device log
    this.log(
      `[DAILY SUMMARY at 00:00] End of day totals before reset: `
      + `Daily Charged: ${dailyCharged} kWh, `
      + `Daily Discharged: ${dailyDischarged} kWh`,
    );

    // Optionally log to timeline if setting enabled
    const logToTimeline = this.getSetting('log_daily_totals_to_timeline') as boolean | undefined;
    if (logToTimeline !== false) { // default true if undefined
      try {
        await this.homey.notifications.createNotification({
          excerpt: `${this.getName()}: Daily totals - Charged: ${dailyCharged} kWh, Discharged: ${dailyDischarged} kWh`,
        });
        this.log('Daily totals logged to timeline');
      } catch (error) {
        this.error('Failed to create daily totals timeline notification:', error);
      }
    }
  }

  /**
   * Handle AFTER automatic baseline reset (called at 00:00 AFTER reset)
   * Callback from BatteryMetricsStore when automatic daily reset occurs
   * Updates capabilities to reflect new baseline immediately
   */
  private async handleAutomaticBaselineReset(): Promise<void> {
    ...
    try {
      // Get aggregated metrics after reset and update UI
      ...
      this.log(
        `Baseline reset - Capabilities updated: `
        + `startOfDay charged=${aggregated.startOfDayChargedKwh.toFixed(2)} kWh, `
        + `current charged=${aggregated.currentChargedKwh.toFixed(2)} kWh`,
      );
    } catch (error) {
      this.error('Error updating capabilities after automatic baseline reset:', error);
    }
  }
  ...
  protected async handleManualResetBaseline(): Promise<void> {
    ...
    this.manualResetTimeout = this.homey.setTimeout(async () => {
      ...
      try {
        ...
        // Update external battery aggregated capabilities after reset
        try {
          const aggregated = await this.externalBatteryMetrics!.getAggregatedMetrics();
          await this.setCapabilityValue('external_battery_daily_charged', aggregated.dailyChargedKwh);
          await this.setCapabilityValue('external_battery_daily_discharged', aggregated.dailyDischargedKwh);
          ...
          await this.setCapabilityValue('external_battery_percentage', aggregated.averageBatteryPercentage);
          await this.setCapabilityValue('external_battery_count', aggregated.batteryCount);
          this.log('[BASELINE] External battery capabilities updated after manual reset');
        } catch (error) {
          this.error('[BASELINE] Error updating external battery capabilities after reset:', error);
        }
      } catch (error) {
        this.error('[BASELINE] Error during emergency reset baseline:', error);
      }
